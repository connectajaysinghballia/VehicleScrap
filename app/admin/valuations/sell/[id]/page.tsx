...
export default function SellDetailPage({ params }: { params: { id: string } }) {
    ...
    const fetchRequest = async () => {
        try {
            const res = await fetch(`/api/admin/valuations/sell/${params.id}`)
            if (res.ok) {
                ...
                if (data.status === "pending") {
                    await fetch("/api/admin/requests/approve", {
                        ...
                        body: JSON.stringify({ id: params.id, type: "sell", status: "reviewed" })
                    })
    ...
    const handleApprove = async () => {
        ...
        try {
            const res = await fetch("/api/admin/requests/approve", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ id: params.id, type: "sell" })
            })
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/requests/delete?id=${params.id}&type=sell`, {
                method: "DELETE"
            })
